...
<div class='grid4'>
  ...
  <div class='card'><span class='mini'>模块</span><div class='metric'>{metrics['module_count']}</div></div>
  <div class='card'><span class='mini'>基础题</span><div class='metric'>{metrics['basic_count']}</div></div>
  <div class='card'><span class='mini'>高级题</span><div class='metric'>{metrics['advanced_count']}</div></div>
</div>
""", unsafe_allow_html=True)
    ...
    st.dataframe(
        filtered[["exercise_id", "module", "difficulty", "related_task", "required_output"]],
...
""", unsafe_allow_html=True)
        ...
        if c2.button("标记练习完成", type="primary", use_container_width=True, key=f"done_{exercise['exercise_id']}"):
            target = current_learner_name or LEARNERS[LEARNERS["learner_id"] == _selected_learner_id()].iloc[0]["learner_name"]
            _record_event(f"{target} 完成练习题 {exercise['exercise_id']}")
            st.success("已写入业务操作记录。")
    with right:
...
def render_public_site() -> None:
    ...
    st.markdown("""
<div class='top'><div class='brand'>AI Skill Growth OS<small>Concrete Business Ops · JHC Java Training Demo</small></div><div class='nav'><span class='pill'>Clients</span><span class='pill'>Cohorts</span><span class='pill'>Proof Tasks</span><span class='pill'>Practice Bank</span><span class='pill hot'>进入业务系统</span></div></div>
""", unsafe_allow_html=True)
    left, right = st.columns([1.2, .8])
    with left:
        st.markdown("""
<div class='hero'><span class='pill hot'>业务运营版 · v4.9.1</span><h1>从展示页变成<br><span>训练业务操作网站</span></h1><p>当前测试业务固定为：信华信日本业务部 2026 Java新人训练营。系统围绕真实业务对象运行：客户、班级、学员、Proof Task、练习题、Agent Review、Skill Proof File、咨询线索和服务包。</p></div>
""", unsafe_allow_html=True)
        st.markdown(f"""
<div class='grid4'>
  ...
  <div class='card'><span class='mini'>待Review</span><div class='metric'>{metrics['pending_reviews']}</div></div>
...
""", unsafe_allow_html=True)
        ...
        st.markdown("<div class='grid3'><div class='card'><h3>客户</h3><p>信华信日本业务部、比特顽童教育。</p></div><div class='card'><h3>班级</h3><p>2026 JHC Java新人训练营，13人，3个月交付。</p></div><div class='card'><h3>练习题</h3><p>登录测试、订单流转、日语Q&A、错误定位、Founder决策、服务包销售。</p></div></div>", unsafe_allow_html=True)
        st.markdown("<div class='section'>业务训练闭环</div>", unsafe_allow_html=True)
        st.markdown("<div class='grid4'><div class='flow-step'><b>1. 建班</b><br><span class='mini'>客户、班级、学员。</span></div><div class='flow-step'><b>2. 布置练习</b><br><span class='mini'>题目绑定真实业务场景。</span></div><div class='flow-step'><b>3. Review</b><br><span class='mini'>Agent Review + Founder决策。</span></div><div class='flow-step'><b>4. 转化</b><br><span class='mini'>Proof Files 和服务包线索。</span></div></div>", unsafe_allow_html=True)
    with right:
        st.markdown("<div class='login-box'><div class='panel'><h2>进入业务系统</h2><p>选择学员或Founder身份，使用内置测试数据操作 JHC Java 新人训练业务和练习题库。</p></div>", unsafe_allow_html=True)
        ...
        if c2.button("Founder运营Demo"):
            ...
            st.rerun()
        st.markdown("<div class='card'><h3>推荐操作顺序</h3><p>1. Founder运营Demo<br>2. 进入 Proof Tasks<br>3. 按模块筛选练习题<br>4. 布置给当前学员<br>5. 查看标准答案和评分标准<br>6. 进入 Review Queue 处理结果</p></div></div>", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    nav = [("dashboard", "业务首页"), ("tasks", "Proof Tasks/练习题"), ("portfolio", "Proof Files"), ("consult", "服务包/Leads")]
...
def student_dashboard() -> None:
    ...
    st.markdown(f"""
<div class='hero'><span class='pill hot'>学员业务首页 · v4.9.1</span><h1>{learner['learner_name']}：今天完成<br><span>{current_task['proof_task']}</span></h1><p>班级：{cohort['cohort_name']}。这是具体业务数据，不是空白展示页。你可以查看任务、完成练习、修改草稿、请求Review并提交。</p><span class='pill'>小组：{learner['group']}</span><span class='pill'>状态：{learner['status']}</span><span class='pill'>完成任务：{learner['tasks_done']}</span><span class='pill'>Proof Files：{learner['proof_files']}</span></div>
""", unsafe_allow_html=True)
    st.markdown(f"""
<div class='grid4'>
  ...
  <div class='card'><span class='mini'>练习题库</span><div class='metric'>{len(EXERCISES)}</div></div>
  <div class='card'><span class='mini'>当前分数</span><div class='metric'>{current_task['proof_score']}</div></div>
</div>
""", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    if c1.button("打开 Proof Task / 练习题", type="primary", use_container_width=True):
        set_view("tasks")
        st.rerun()
...
def founder_dashboard() -> None:
    ...
    exercise_metrics = get_exercise_metrics()
    st.markdown("""
<div class='hero'><span class='pill hot'>Founder运营首页 · v4.9.1</span><h1>现在是训练业务运营系统，<br><span>不是展示Demo</span></h1><p>这里直接管理客户、班级、学员、练习题、Proof Task、Review Queue、Proof Files 和 Leads。</p></div>
""", unsafe_allow_html=True)
    st.markdown(f"""
<div class='grid4'>
  <div class='card decision'><span class='mini'>训练学员</span><div class='metric'>{metrics['learners']}</div></div>
  <div class='card decision'><span class='mini'>业务练习题</span><div class='metric'>{exercise_metrics['exercise_count']}</div></div>
...
""", unsafe_allow_html=True)
    ...
    with left:
        ...
        if c1.button("处理 Review Queue", type="primary", use_container_width=True):
            set_view("queue")
            st.rerun()
        if c2.button("打开练习题库", use_container_width=True):
            set_view("tasks")
            st.rerun()
    with right:
        st.markdown("<div class='section'>今日运营建议</div>", unsafe_allow_html=True)
        st.markdown("<div class='card'><h3>先布置练习题，再处理Review</h3><p>鈴木美咲的订单状态流转拆解已达81分，可用 Founder 决策题继续训练。</p></div>", unsafe_allow_html=True)
        st.markdown(_event_panel(), unsafe_allow_html=True)
...
def tasks_page() -> None:
    ...
    st.markdown("<div class='panel'><span class='pill hot'>Proof Task + 业务练习题</span><h2>按班级和学员处理真实训练任务</h2><p>这里的任务和练习题都绑定到具体业务场景、交付物、标准答案和评分标准。</p></div>", unsafe_allow_html=True)
