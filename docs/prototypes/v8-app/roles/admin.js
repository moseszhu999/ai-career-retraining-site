...
window.ProofSkillRoles.admin = {
  ...
  render(state) {
    ...
    const quizRows = data.quizQuestions.map((q, index) => `
      <tr>
        ...
        <td><span class="badge text-bg-success">answer key ready</span></td>
      </tr>
    `).join('');

    const taskRows = data.projectTasks.map((task) => `
      <tr>
        <td><strong>${task.title}</strong><br><small class="text-secondary">${task.credentialId}</small></td>
        <td>${task.difficulty}</td>
        <td>${task.estimatedTime}</td>
        <td>${task.evidenceOutputs.length}</td>
      </tr>
    `).join('');
      ...
      <div class="card mb-4 border-primary">
        ...
        <div class="card-body">
          <div class="row g-3 mb-3">
            <div class="col-lg-4">
              <label class="form-label small text-secondary">Credential path</label>
              <select class="form-select">
                <option>AI Data Analysis Assistant</option>
                <option>AI Trade Documentation Assistant</option>
              </select>
            </div>
            <div class="col-lg-4">
              <label class="form-label small text-secondary">Publish mode</label>
              <select class="form-select">
                <option>Draft -> Review -> Published</option>
                <option>Pilot only</option>
                <option>Archive old version</option>
              </select>
            </div>
            <div class="col-lg-4">
              <label class="form-label small text-secondary">Content policy</label>
              <input class="form-control" value="No private learner answers inside published curriculum">
            </div>
          </div>
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
              <thead><tr><th>Module</th><th>Type</th><th>Duration</th><th>Status</th><th>Action</th></tr></thead>
              <tbody>${lessonRows}</tbody>
            </table>
          </div>
          <div class="d-flex flex-wrap gap-2 mt-3">
            <button id="adminPublishCurriculum" class="btn btn-primary">Publish Curriculum Version Mock</button>
            <button class="btn btn-outline-primary">Add Lesson Mock</button>
            <button class="btn btn-outline-secondary">Archive Draft Mock</button>
          </div>
        </div>
      </div>
      ...
      <div class="row g-3 mb-4">
        <div class="col-xl-6">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Quiz Bank</div>
            <div class="card-body table-responsive">
              <table class="table table-sm table-hover align-middle mb-0">
                <thead><tr><th>ID</th><th>Question</th><th>Choices</th><th>Status</th></tr></thead>
        ...
        <div class="col-xl-6">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Practice Lab Templates</div>
            <div class="card-body table-responsive">
              <table class="table table-sm table-hover align-middle mb-0">
                <thead><tr><th>Task</th><th>Difficulty</th><th>Time</th><th>Evidence outputs</th></tr></thead>
  ...
  bind() {
    ...
    document.getElementById('adminPublishCurriculum')?.addEventListener('click', () => window.ProofSkillApp.mutate((s) => {
      s.curriculumVersion = `curriculum-${Date.now()}`;
    }, 'Admin published curriculum version mock'));
  }
};
