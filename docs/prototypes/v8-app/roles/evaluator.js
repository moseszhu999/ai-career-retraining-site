  ...
  render(state) {
    ...
    return `
      ...
      <div class="row g-3 mb-4">
        <div class="col-xl-5">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Review Assignment · ${task.title}</div>
            <div class="card-body">
              <p class="text-secondary">Difficulty: ${task.difficulty} · Estimated time: ${task.estimatedTime}</p>
              <div class="alert alert-warning"><strong>Risk flags:</strong> quantity mismatch, ship-date mismatch, replacement quantity pending human confirmation.</div>
              <ul class="list-group mb-3">${task.checklist.map((item) => `<li class="list-group-item d-flex justify-content-between"><span>${item}</span><span class="badge text-bg-success">checked</span></li>`).join('')}</ul>
        ...
        <div class="col-xl-7">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Rubric Scoring</div>
            <div class="card-body table-responsive">
              <table class="table table-hover align-middle mb-3">
                <thead><tr><th>Rubric item</th><th>Weight</th><th>Score</th><th>Review flag</th></tr></thead>
                <tbody>${rubricRows}</tbody>
              </table>
              <label class="form-label small text-secondary">Evaluator comment</label>
              <textarea class="form-control" rows="3">Strong evidence package. Main issue is that replacement quantity must be routed to a human approver before final certificate issuance.</textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row g-3">
        <div class="col-lg-6">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Evidence Summary</div>
            <div class="card-body">
              <dl class="row small mb-0">
                <dt class="col-5">Learner</dt><dd class="col-7">Aarav Patel</dd>
                <dt class="col-5">Credential</dt><dd class="col-7">AI Trade Documentation Assistant</dd>
                <dt class="col-5">Evidence package</dt><dd class="col-7">exception-table.xlsx · document-review-note.md · risk-summary.pdf</dd>
                <dt class="col-5">Boundary</dt><dd class="col-7">This review is not legal, tax, or financial advice.</dd>
              </dl>
            </div>
          </div>
        </div>
