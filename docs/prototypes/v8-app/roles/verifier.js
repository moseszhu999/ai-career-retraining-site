...
window.ProofSkillRoles.verifier = {
  ...
  render(state) {
    ...
    const roleRows = data.employerRoles.map((role) => `
      <tr>
        <td><strong>${role.title}</strong><br><small class="text-secondary">${role.companyType}</small></td>
        <td>${role.preferredCredential}</td>
        <td>${role.minimumTrust}</td>
      </tr>
    `).join('');
    ...
    const fitRows = data.jobFitMatrix.map((row) => `
      <tr>
        <td>${row.signal}</td>
        <td>${row.required}</td>
        <td style="width: 30%"><div class="progress"><div class="progress-bar" style="width:${row.candidate}%">${row.candidate}</div></div></td>
        <td><code>${row.evidence}</code></td>
      </tr>
    `).join('');

    const riskRows = data.hiringRiskNotes.map((item) => `
      <tr><td>${item.risk}</td><td>${item.mitigation}</td></tr>
    `).join('');
    ...
    return `
        ...
        <div class="col-lg-5">
          <div class="card h-100">
            ...
            <div class="card-body">
              ...
              <div class="alert alert-info mb-0">Credential verification is a work-readiness signal. It is not an automatic decision system.</div>
            </div>
          </div>
        </div>
        <div class="col-lg-7">
          <div class="card h-100 border-primary">
            <div class="card-header bg-white fw-bold">Candidate Skill Signal Profile</div>
            <div class="card-body">
              <div class="d-flex justify-content-between gap-3 flex-wrap mb-3">
                <div>
                  <h4 class="mb-1">${candidate.name}</h4>
                  <div class="text-secondary small">${candidate.wallet}</div>
                  <div class="mt-2"><span class="badge text-bg-primary">${candidate.credential}</span> <span class="badge text-bg-success">${candidate.trustLevel}</span></div>
                </div>
                <div class="text-end">
                  <div class="small text-uppercase text-secondary fw-bold">Role-fit signal</div>
                  <div class="display-6">${candidate.overallFit}</div>
                  <span class="badge text-bg-success">${candidate.recommendedRole}</span>
                </div>
              </div>
              <div class="row g-3">
                <div class="col-md-6"><div class="small text-uppercase text-secondary fw-bold mb-2">Strengths</div><ul class="small mb-0">${candidate.strengths.map((item) => `<li>${item}</li>`).join('')}</ul></div>
                <div class="col-md-6"><div class="small text-uppercase text-secondary fw-bold mb-2">Limits</div><ul class="small mb-0">${candidate.limits.map((item) => `<li>${item}</li>`).join('')}</ul></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="row g-3 mb-4">
        <div class="col-xl-7">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Role-fit Signal Matrix</div>
            <div class="card-body table-responsive">
              <table class="table table-hover align-middle mb-0">
                <thead><tr><th>Signal</th><th>Required</th><th>Candidate</th><th>Evidence</th></tr></thead>
                <tbody>${fitRows}</tbody>
              </table>
            </div>
          </div>
        ...
        <div class="col-xl-5">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Role Requirement Reference</div>
            <div class="card-body table-responsive">
              <table class="table table-sm table-hover align-middle mb-0">
                <thead><tr><th>Role</th><th>Credential</th><th>Trust</th></tr></thead>
      ...
      <div class="row g-3 mb-4">
        <div class="col-lg-6">
          <div class="card h-100">
            <div class="card-header bg-white fw-bold">Use-boundary Notes</div>
            <div class="card-body table-responsive">
              <table class="table table-sm table-hover align-middle mb-0">
                <thead><tr><th>Boundary</th><th>Handling</th></tr></thead>
